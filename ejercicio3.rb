...
#2. Opción 1: Permite ingresar los datos de una persona: -Nombre- Edad- Comuna- Género
...
def seleccionar_persona(lista, mensaje ='Seleccione una opción')
  if lista.length.zero?
    puts 'ERROR: No hay personas seleccionadas.'
    -1
  else
    loop do
      puts mensaje
...
def editar_persona(lista)
  ...
  opt = seleccionar_persona(lista, 'Seleccione la persona a editar:')
  if opt >= 0
    editar = true
    while editar
...
def eliminar_persona(lista)
  ...
  opt = seleccionar_persona(lista, 'Seleccione la persona a eliminar:')
  if opt >= 0
